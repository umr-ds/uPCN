 ...
 *
 * All lookup tables are **reflected**. This removes the requirement to reverse
 * the input data.
 *
...
/**
 * Reflected CRC-32 Ethernet lookup table
 *
 * Parameters:
 *   Polynom: 0x04c11db7
 *   Reflect input data: TRUE
...
void bundle7_crc_read(struct bundle7_crc_stream *crc, uint8_t byte)
{
	...
	// Shift out the index and XOR-ing the remainder from the lookup table
	crc->checksum = (crc->checksum >> 8) ^ (crc->table[index]);
}


void bundle7_crc_done(struct bundle7_crc_stream *crc)
{
	if (crc->table == crc16_x25_table)
		// Final XOR for CRC-16 X.25
		crc->checksum ^= 0xffff;
	else
		// Final XOR for CRC-32 Ethernet
...
void bundle7_crc_init(struct bundle7_crc_stream *crc,
	enum bundle7_crc_version version)
{
	switch (version) {
	case BUNDLE_V7_CRC16:
		// Initial value as defined in CRC-16 X.25
		crc->checksum = 0xffff;
		crc->table = crc16_x25_table;
		break;
	default:
		// Initial value as defined in CRC-32 Ethernet
		crc->checksum = 0xffffffff;
		crc->table = crc32_table;
		break;
	}
}
