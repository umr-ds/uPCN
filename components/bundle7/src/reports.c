...
static struct bundle *wrap_administrative_record(const struct bundle *bundle,
	uint8_t *payload, size_t payload_size)
{
	struct bundle *record = bundle_init();

	if (record == NULL) {
		bundle7_free(payload);
		return NULL;
	}

	record->proc_flags |= BUNDLE_FLAG_ADMINISTRATIVE_RECORD;

	// Creation time
	record->creation_timestamp = hal_time_get_timestamp_s();
	record->sequence_number = 1;

	// Lifetime
	int64_t lifetime = bundle->lifetime - record->creation_timestamp
	...
	record->payload_block = bundle_block_create(BUNDLE_BLOCK_TYPE_PAYLOAD);
	record->blocks = bundle_block_entry_create(record->payload_block);

	if (record->payload_block == NULL || record->blocks == NULL) {
		bundle_free(record);
		bundle7_free(payload);
		return NULL;
	}

	record->payload_block->data = payload;
	record->payload_block->length = payload_size;
	...
	char *source)
{
	...
	payload = bundle7_malloc(payload_size);
	if (payload == NULL)
		return NULL;
	...
	uint8_t *compress = bundle7_realloc(payload, written);
	...
	// Wrapping payload into a bundle
	struct bundle *record_bundle = wrap_administrative_record(
		bundle, compress, written);

	if (record_bundle == NULL)
		return NULL;

	// Set destination and source EID
	record_bundle->source =
		eidmanager_alloc_ref(source, false);
	record_bundle->destination =
		eidmanager_alloc_ref(bundle->report_to, false);

	return record_bundle;
}


struct bundle *generate_custody_signal(const struct bundle *bundle,
	const struct bundle_custody_signal *prototype)
{
	// Custody is not supported for BPbis >= draft v7
	return NULL;
}
...
error_t parse_last_fields(struct record_parser *state, CborValue *it)
{
	...
	state->record->bundle_source_eid = eidmanager_alloc_ref(eid, true);
...
error_t status_report(struct record_parser *state, CborValue *it)
{
	...
	state->record->status_report = bundle7_malloc(
	...
	const struct bundle * const record_bundle)
{
	...
	if (record_bundle->payload_block->length == 0)
		return NULL;

	// Initialize administrative record
	struct bundle_administrative_record *record = bundle7_malloc(
	...
	err = cbor_parser_init(
		record_bundle->payload_block->data,
		record_bundle->payload_block->length,
		0, &parser, &it
	);
	if (err) {
		bundle7_free(record);
