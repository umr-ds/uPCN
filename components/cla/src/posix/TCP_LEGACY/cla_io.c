		      ...
		      const size_t length, const enum comm_type type)
{
	...
	snd_packet.sndbuffer[2] = (uint8_t)type;
	snd_packet.sndbuffer[3] = (length & 0xFF00) >> 8;
	snd_packet.sndbuffer[4] = length & 0x00FF;
	snd_packet.packet_position_ptr = &snd_packet.sndbuffer[5];
	snd_packet.length = length+5;
