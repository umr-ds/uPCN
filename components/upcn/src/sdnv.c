...
static inline int sdnv_validate_byte_u32
	(struct sdnv_state *state, uint32_t *value, uint8_t byte)
{
...
static inline int sdnv_validate_byte_u16
	(struct sdnv_state *state, uint16_t *value, uint8_t byte)
{
	/* max. SDNV: 0x83FF7F */
...
static inline int sdnv_validate_byte_u64
	(struct sdnv_state *state, uint64_t *value, uint8_t byte)
{
...
do { \
	if (!validate_function(state, value, byte)) { \
