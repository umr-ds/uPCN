	...
	Semaphore_t clist_semaphore, struct contact_list **clistptr)
{
	...
	LOG("RouterOptimizer: Updated config");
...
static void router_optimizer_task(void *param)
{
	...
	LOG("RouterOptimizer: Started successfully");
	for (;;) {
		...
		if (router_optimization_affordable(p->clist_ptr)) {
			...
			if (opt != 0)
				LOGA("RouterOptimizer: Optimized bundles",
					opt, LOG_NO_ITEM);
...
/*static int routed_bdl_cmp(const void *_a, const void *_b);*/
	...
	struct contact_list *global_clist, QueueIdentifier_t router_queue)
{
	...
	while (cur_elem != NULL) {
		...
		if (popt_c != 0) {
			/* FIXME: This seems to crash the SoC sometimes... */
			/*qsort(popt, popt_c, sizeof(void *), routed_bdl_cmp);*/
			for (i = 0; i < popt_c; i++)
