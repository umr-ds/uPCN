...
static void begin_read_gs_eid(struct config_parser *parser)
{
	...
	const uint8_t *buffer, size_t length)
{
	...
	while (i < length) {
		...
		if (parser->basedata->status != PARSER_STATUS_GOOD
		    && parser->basedata->status != PARSER_STATUS_DONE) {
			LOGF("ConfigAgentParser: parser status " \
			     "was not good at %c -> reset " \
			     "parser", buffer[i]);
...
static void process_unmanaged_eids(struct ground_station *gs, const bool add)
{
