...
static void write(const void *cla_obj, const void *data, const size_t length)
{
	TEST_ASSERT_TRUE_MESSAGE(
		output_bytes + length <= len_simple_bundle,
		"CBOR too long");

	TEST_ASSERT_EQUAL_INT8_ARRAY(cbor_simple_bundle + output_bytes,
		data, length);
...
TEST(bundle7Serializer, simple_bundle)
{
	...
	block->data = bundle7_malloc(sizeof(previous_node));
	...
	block->data = bundle7_malloc(sizeof(hop_count));
	TEST_ASSERT_NOT_NULL(block->data);
	...
	block->data = bundle7_malloc(sizeof(bundle_age));
	TEST_ASSERT_NOT_NULL(block->data);
...
static void write_crc16_primary_block(const void *cla_obj, const void *data,
	const size_t len)
{
	TEST_ASSERT_TRUE_MESSAGE(
	...
	TEST_ASSERT_EQUAL_INT8_ARRAY(cbor_crc16_primary_block + output_bytes,
		data, len);

	output_bytes += len;
}

static void write_crc16_payload_block(const void *cla_obj, const void *data,
	const size_t len)
{
	TEST_ASSERT_TRUE_MESSAGE(
	...
	TEST_ASSERT_EQUAL_INT8_ARRAY(cbor_crc16_payload_block + output_bytes,
		data, len);
...
TEST(bundle7Serializer, crc16_generation)
{
	...
	bundle->destination = eidmanager_alloc_ref("dtn:GS2", false);
	bundle->source = eidmanager_alloc_ref("dtn:none", false);
	bundle->report_to = eidmanager_alloc_ref("dtn:none", false);
	...
	uint8_t empty[] = { 0x40 }; // Empty CBOR byte string

	block->number = 0;
	block->crc_type = BUNDLE_CRC_TYPE_NONE;
	block->length = sizeof(empty);
	block->data = bundle7_malloc(sizeof(empty));
	TEST_ASSERT_NOT_NULL(block->data);
	memcpy(block->data, empty, sizeof(empty));
	bundle->payload_block = block;
...
static void write_crc32_primary_block(const void *cla_obj, const void *data,
	const size_t len)
{
	TEST_ASSERT_TRUE_MESSAGE(
	...
	TEST_ASSERT_EQUAL_INT8_ARRAY(cbor_crc32_primary_block + output_bytes,
		data, len);

	output_bytes += len;
}

static void write_crc32_payload_block(const void *cla_obj, const void *data,
	const size_t len)
{
	TEST_ASSERT_TRUE_MESSAGE(
	...
	TEST_ASSERT_EQUAL_INT8_ARRAY(cbor_crc32_payload_block + output_bytes,
		data, len);
...
TEST(bundle7Serializer, hop_count)
{
	...
	uint8_t *buffer = bundle7_malloc(BUNDLE7_HOP_COUNT_MAX_ENCODED_SIZE);
