...
static void send_callback(struct bundle *_bundle)
{
	bundle = _bundle;
}
...
static void parse_bundle_chunked(const size_t chunk_size)
{
	...
	struct parser *parser = bundle7_parser_init(&state, &send_callback);
	...
	uint8_t payload[] = {
		0x4c, // CBOR byte string with length 12
		// "Hello world!"
		'H', 'e', 'l', 'l', 'o', ' ', 'w', 'o', 'r', 'l', 'd', '!',
	};
...
TEST(bundle7Parser, crc16_verification)
{
	...
	TEST_ASSERT_EQUAL(bundle->payload_block->crc.checksum, 0x878e);
...
TEST(bundle7Parser, crc32_verification)
{
	...
	TEST_ASSERT_EQUAL(bundle->crc.checksum, 0xc555fd3b);
	...
	TEST_ASSERT_EQUAL(bundle->payload_block->crc.checksum, 0xc703579d);
...
TEST(bundle7Parser, status_report_parser)
{
	...
	payload->data = bundle7_malloc(payload->length);
	...
	struct bundle_administrative_record *record =
		bundle7_parse_administrative_record(bundle);
