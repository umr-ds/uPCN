...
__ALIGN_BEGIN uint8_t USB_Rx_Buffer   [CDC_DATA_MAX_PACKET_SIZE] __ALIGN_END ;
  ...
  #endif
#endif /* USB_OTG_HS_INTERNAL_DMA_ENABLED */
__ALIGN_BEGIN uint8_t APP_Rx_Buffer   [CDC_DATA_IN_PACKET_SIZE] __ALIGN_END ;
...
/**
  ...
  *
  * NOTE:
  * The transfer is initiated (and the first packet sent) by Handle_USBAsynchXfer.
  * This function transfers data until the buffer is empty and completes
  * if necessary (sent data is a multiple of CDC_DATA_IN_PACKET_SIZE) with a ZLP.
  */
uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t epnum)
{
	uint32_t APP_Rx_length = 0;
	uint8_t More_Data_Avail = 0;

	if (USB_Tx_State == USB_CDC_BUSY) {
		APP_FOPS.pIf_DataTx(
			(uint8_t *)APP_Rx_Buffer, CDC_DATA_IN_PACKET_SIZE,
			&APP_Rx_length, &More_Data_Avail);
		if (APP_Rx_length == 0)
		{
			/* This _should_ never happen. Nobody else should read from the queue! */
			USB_Tx_State = USB_CDC_IDLE;
		}
		else
		{
			/*
			 * If multiple of packet size and no more data available,
			 * send a ZLP during next transfer to complete it;
			 * if packet was smaller, go to idle immediately to prevent
			 * sending further data when a packet smaller than
			 * CDC_DATA_IN_PACKET_SIZE has been sent but the queue was filled
			 * in the meantime.
			 */
			if (APP_Rx_length == CDC_DATA_IN_PACKET_SIZE && !More_Data_Avail)
...
/**
  ...
  *
  * NOTE:
  * This function begins the transfer. It checks the available data to be sent.
  * If more data than CDC_DATA_IN_PACKET_SIZE is available, this will be sent in
  * usbd_cdc_DataIn above. If not, a Zero-Length-Packet will be sent.
  */
static void Handle_USBAsynchXfer(void *pdev)
{
	uint32_t APP_Rx_length = 0;
	...
	if (USB_Tx_State == USB_CDC_IDLE) {
		...
		if (APP_Rx_length < CDC_DATA_IN_PACKET_SIZE) {
			...
			if (APP_Rx_length == 0)
				return;
		} else if (APP_Rx_length == CDC_DATA_IN_PACKET_SIZE && !More_Data_Avail) {
			USB_Tx_State = USB_CDC_ZLP;
		} else {
